...
bool Pig::useK() {
    // TODO: 补全代码
    Pig* nextPig=getNextPig();
    if(this->type=='M') { //主
        if(nextPig->jumpType=='F' || nextPig->jumpType=='f') { //下家已经跳反或者是类反
            jump();
            nextPig->cost(this,'D');
            return true;
        }
    }
    else if(this->type=='Z') { //忠
        if(nextPig->jumpType=='F') { //下家已经跳反
            jump();
            nextPig->cost(this,'D');
            return true;
        }
...
bool Pig::useF() {
    ...
    if(this->type=='M') { //主
        bool noJumpF=true;
        for(Pig* nextPig=getNextPig();nextPig!=this;nextPig=nextPig->getNextPig()) {
            if(nextPig->jumpType=='F') { //已经跳反
                noJumpF=false;
                jump();
                if(!nextPig->findJ(this)) { //没有求到无懈可击
                    //轮流出杀
                    while(1) {
                        if(!nextPig->del('K')) {
                            nextPig->hurt(this);
        ...
        if(noJumpF) { //没有跳反
            for(Pig* nextPig=getNextPig();nextPig!=this;nextPig=nextPig->getNextPig()) {
                if(nextPig->jumpType=='f') { //类反
                    jump();
                    if(!nextPig->findJ(this)) { //没有求到无懈可击
                        //轮流出杀
                        if(nextPig->type=='Z') { //类反的身份是忠
                            nextPig->hurt(this);
                        }
                        else { //类反的身份是反
                            while(1) {
...
bool Pig::useN() {
    for (Pig* nxt = getNextPig(); nxt != this; nxt = nxt->getNextPig()) {
        ...
        if(!nxt->findJ(this)) {
            if(!nxt->cost(this,'K')) {
                if(jumpType==0 && nxt->type=='M') jumpType='f';
            }
        }
    }
    return true;
